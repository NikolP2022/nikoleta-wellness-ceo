...
async function renderHome(){
 const root=document.querySelector('#app'), main=root?.querySelector('main'); if(!main||location.hash==='#appointments')return;
 let items=[];
...
new MutationObserver(()=>{if(!mounted&&!location.hash&&document.querySelector('#app main'))setTimeout(tryRender,100)}).observe(document.body,{childList:true,subtree:true});
})();