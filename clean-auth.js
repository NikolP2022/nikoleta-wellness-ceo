(()=>{
  ...
  async function act(mode){
    ...
    try{
      if(mode==='forgot'){msg('⏳ Στέλνω email…');const {error}=await client.auth.resetPasswordForEmail(email,{redirectTo:APP+'?password-reset=1'});if(error)throw error;return msg('✅ Έλεγξε το email σου για αλλαγή κωδικού.')}
      msg('⏳ Γίνεται σύνδεση…');let r;
