...
function resolveClient(name,list,current){const q=String(name||'').trim().toLowerCase();if(!q)return current?.id||null;const exact=list.filter(x=>String(x.name||'').trim().toLowerCase()===q);if(exact.length===1)return exact[0].id;const partial=list.filter(x=>String(x.name||'').toLowerCase().includes(q));return partial.length===1?partial[0].id:null}
function openEditor(existing,current,list){
 ...
 m.querySelector('form').onsubmit=async e=>{
   ...
   const client_id=resolveClient(f.client_name.value,list,current),errs=[];
   if(!client_id)errs.push('Δεν βρέθηκε μοναδικός πελάτης με αυτό το όνομα. Διάλεξε από τη λίστα ή γράψε ακριβώς το υπάρχον όνομα.');
   ...
   const p={user_id:u.id,client_id,measurement_date:f.measurement_date.value,measurement_time:f.measurement_time.value||nowTime(),notes:f.notes.value||null};
