...
function cleanNewClient(m){
  ...
  const name=form.elements.name;if(name){name.value='';name.placeholder='Γράψε το όνομα του πελάτη';name.removeAttribute('value');}
}
function scan(root=document){
  root.querySelectorAll?.('.ap130-modal,.nw186-modal').forEach(m=>{cleanNewAppointment(m);cleanNewClient(m)});
}
